...
let purchaseBtn = document.getElementById("buy");
let isLoggedIn = JSON.parse(localStorage.getItem("loggedIn"));
let userActionBtn = document.querySelector(".cta>a");
...
if (isLoggedIn) {
  userActionBtn.innerHTML = `${userLogin.name}  <i class="fa-solid fa-right-from-bracket" style="color: #ffffff;"></i>`;
  ...
  userActionBtn.innerText = "Log In";
  userActionBtn.addEventListener("click", (event) => {
...
function populateWishlistContent(cart) {
  ...
  if (isLoggedIn) {
    ...
    if (cart.length > 0) {
      ...
      purchaseBtn.classList.add("show-btn");
      purchaseBtn.classList.remove("hide-btn");
    } else {
      let emptyMsg = document.createElement("h3");
      emptyMsg.innerText = "Your Cart is empty";
      purchaseBtn.classList.add("hide-btn");
      purchaseBtn.classList.remove("show-btn");
